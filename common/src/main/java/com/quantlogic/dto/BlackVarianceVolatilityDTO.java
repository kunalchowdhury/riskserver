...
public class BlackVarianceVolatilityDTO implements DTOEntity {
    ...
    public BlackVarianceVolatilityDTO() {
    }

    public BlackVarianceVolatilityDTO(long valuationDate,
                                      byte calendar,
                                      long[] expirations,
                                      double[] strikes,
                                      byte curDayCounter,
    ...
    public String toString() {
        return "BlackVarianceVolatilityDTO{" +
    ...
    public boolean equals(Object o) {
        ...
        BlackVarianceVolatilityDTO that = (BlackVarianceVolatilityDTO) o;
        return valuationDate == that.valuationDate && calendar == that.calendar && curDayCounter == that.curDayCounter
                && version == that.version && shardId == that.shardId && Arrays.equals(expirations, that.expirations)
                && Arrays.equals(strikes, that.strikes) && Arrays.deepEquals(vols, that.vols) && name.equals(that.name);
    }

    @Override
    public int hashCode() {
        int result = Objects.hash(valuationDate, calendar, curDayCounter, version, shardId, name);
        result = 31 * result + Arrays.hashCode(expirations);
        result = 31 * result + Arrays.hashCode(strikes);
        result = 31 * result + Arrays.deepHashCode(vols);
        return result;
    }
}
